...
## Cavekit Mermaid Ecosystem Integration

Pretty-Mermaid es el **renderer final** en la cadena de mermaid de Cavekit. No valida ni genera diagramas desde cero; los **renderiza** con calidad profesional. Conocé los distintos roles antes de usarlo:

### Role mapping

| Skill | Rol | Relación con Pretty-Mermaid |
|-------|-----|---------------------------|
| `c4-architecture` | Genera diagramas C4 `.mmd` desde análisis de código | Producís `.mmd` → validás con mermaid-fixer → renderizás con Pretty-Mermaid |
| `oh-my-mermaid` | Escanea codebase → genera arquitectura mermaid (`.omm/`) | Pretty-Mermaid complementa: tomás los `.mmd` de `.omm/` y los renderizás con themes |
| `mermaid-fixer` | Valida y repara sintaxis de `.mmd` antes de publicar | **Obligatorio**: validá antes de renderizar. Un `.mmd` roto produce SVG roto. |
| `editorial-diagrams` | Crea diagramas HTML+SVG auto-contenidos desde cero | Pretty-Mermaid es para `.mmd` existentes; editorial-diagrams para diagramas nuevos con interactividad |
| `cavekit-docgen` | Genera wiki/docs embebidos | Pretty-Mermaid pre-renderiza SVGs que cavekit-docgen puede embebir como imágenes fijas |

### Recomendado: pipeline Cavekit completo

```bash
# 1. Generar diagramas (elige una opción)
/c4-architecture      # → .c4/*.mmd  (análisis de código)
# ó
/omm-scan            # → .omm/**/*.mmd  (scan de arquitectura)

# 2. Validar sintaxis (obligatorio antes de renderizar)
node <mermaid-fixer-skill>/assets/validate-mermaid.mjs .c4/*.mmd

# 3. Renderizar con themes profesionales
node <pretty-mermaid-skill>/scripts/batch.mjs \
  --input-dir .c4 \
...
# 4. (Opcional) ASCII para README/terminal
node <pretty-mermaid-skill>/scripts/render.mjs \
  --input .c4/context.mmd \
  --format ascii \
  --use-ascii > docs/architecture/context.txt
```

### Caveat: dependencia npm

Pretty-Mermaid requiere `npm install` de `beautiful-mermaid` (única dependencia, transitivamente `@dagrejs/dagre` → `@dagrejs/graphlib`). Esto **rompe el principio "local-first, zero external packages"** de `c4-architecture`. Usalo como **output enhancement** en fase de docgen, no como parte del análisis de código. El autoinstall en primera ejecución está contenido al directorio del skill (`.skkill`), sin tocar el proyecto.
