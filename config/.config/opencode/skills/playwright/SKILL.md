...
- **`references/page-object-model.md`** - POM design pattern and best practices
- **`references/selectors-guide.md`** - Selector priority and resilient locators
- **`references/mcp-workflow.md`** - MCP exploration workflow before writing tests

Code examples in `assets/`:
- **`assets/login-tests-example.spec.ts`** - Complete test suite with multiple scenarios
- **`assets/helpers-example.ts`** - Test data generation and setup utilities
- **`assets/page-objects-example.ts`** - Page Object implementations (BasePage, LoginPage, HomePage, SignUpPage)
