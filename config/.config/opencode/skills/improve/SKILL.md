...
name: improve
description: >
  Survey any codebase as a senior advisor and produce prioritized, self-contained implementation plans
  for OTHER models/agents to execute. Strictly read-only on source code — never implements, fixes,
  or refactors anything itself. Use when asked to audit a codebase, find improvement opportunities
  (bugs, security, performance, test coverage, tech debt, migrations, DX), suggest features or where
  to take the project next (roadmap, product direction), or generate handoff plans for another
  agent to implement.
license: MIT
metadata:
  author: shadcn
  ...
  imported: "2026-06-11"
  note: Adapted from shadcn/improve (MIT) — content preserved verbatim, metadata aligned with our conventions.
---
...
## Integration with Cavekit

This skill overlaps with the Cavekit workflow (`cavekit-sketch`, `cavekit-map`, `cavekit-check`). Use cases:

- **Standalone codebase audit (no project context)** → use `improve` directly. Best for inherited code, brownfield audits, or "what should we do next" reviews.
- **Building a feature in a known project** → use Cavekit's `sketch` → `map` → `make` → `check` cycle. Plans in `context/kits/`, not `plans/`.
- **Reviewing work before merge** → use `improve branch` to audit what a branch actually changes. Cavekit doesn't have a "branch diff audit" view.
- **Maintaining a plan backlog** → `improve reconcile` for the ad-hoc `plans/` backlog. Cavekit has its own `context/impl/` for in-flight implementation.

The two systems coexist. Don't try to merge them.

## References

- `references/audit-playbook.md` — what to look for, per category, plus the Finding format
- `references/plan-template.md` — the handoff plan template
- `references/closing-the-loop.md` — `execute`, `reconcile`, `--issues` flows

## Origin

Adapted from [shadcn/improve](https://github.com/shadcn/improve) (MIT, 1.9k stars). Content preserved verbatim from the original; only frontmatter metadata, references to the dcfiles/Cavekit ecosystem, and this integration note were added. Source: <https://github.com/shadcn/improve/blob/main/skills/improve/SKILL.md>.
